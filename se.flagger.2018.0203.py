...
#	-
#	- BY == attribute back to 1U RING LLC, and/or Assignee -- All Rights Reserved
#	-
#	- NC == not for your commercial use
#	-	- you can collect consulting fee from your customers
#	-
#	- ND == no derivative allowed - no forking allowed
#	-	( let's combine resources, time, expertise, etc )
#	-
#	-
# =========================
# OpenSource Release Distro	# Selenium Research Demo Project
# =========================
#	https://GitHub.com/seauto
#	https://GitHub.com/seauto/se.bot
#	#
#	# README.2018.0122.txt		 == Installation info
#	#	- Requires 64bit OS
#	#
#	#	- standard CentOS-7.4.1708 ( DVD ) installation w/ online upgrades
#	#
#	#	- Requires ChromeDriver-2.34 or 2.35	# http://chromedriver.storage.googleapis.com/index.html?path=2.35
#	#	- Requires Chrome-2.61 or 2.63		# https://www.google.com/chrome/	# convert *.deb into *.rpm
#	#
#	#
#	# se.flagger.2018.0122.py	# Automatically flag duplicate ads violating CraigsLists ToU in my opionion ..
#	# -----------------------
#	#	flagged articles may be removed when the a critical ratio of the "community" agree it is violating CL ToU
#	#
#	#
#	# se.post.py			# not yet open source - Automatically post new CL ads 
#	# se.renew.py			# not yet open source - Automatically renew all CL ads with renew status
#	#
#
# Default tests
# -------------
#	#
#	# -l city	# City.CraigsList.org
#	#
#	# -c 5		# check top 5 posts
#	# -c 55		# check top 55 posts
#	#
#	# -t		# same md5 of title == duplicate titles
#	# -b		# same md5 of body content == duplicate body - slow to calculate md5 of html page
#	#
#	# -----------------------------------------------------------------
#	# flag duplicate posts in different cities and different categories
#	# -----------------------------------------------------------------
#	se.flagger.py -d -d -d -c 5 -l fresno     -u /d/search/bpa -t -b -f	
#	se.flagger.py -d -d -d -c 5 -l sacramento -u /d/search/cps -t -b -f 		# computer services
#	se.flagger.py -d -d -d -c 5 -l sfbayarea  -u /wanted/search/waa -t -b -f 	# items wanted
#
#
...
# 22-Jan-18 amo Skip calculating body hash is slow - but href still required for user feedback
# 23-Jan-18 amo Flagging works with /opt/google/chrome-2.63 ( was 2.61 ) and /usr/local/bin/chromedriver-2.35 ( was 2.34 )
# 25-Jan-18 amo Released at GitHub.com/seauto/se.bot
# 29-Jan-18 amo Added time-stamp ( posted, updated )
# 02-Feb-18 amo Ignore CL content in duplicate body content - different url and different images to be different 
# 03-Feb-18 amo Ignore more CL content in duplicate body content
#
#
# Known Bugs
# ==========
#	- check time stamps ( same ad re-posted (spam) daily over 30 day period )
#
#
...
#	- also check for 1 line differences	 -- eg. same content except 1 line 
#
#	- but, ads are NOT prohibited if OVER 48 hrs	- no time stamp checking yet
#	- same ad posted more than once in 48hr period
#
#	- prohibited items posts - no content review yet
#
#	- similar title - similar content -- same person per physical (store) address etc
#
#	- use proxy
...
#
#
# Different Browser incompatibilities	- chrome, firefox, seamonkey, konqueror, opera, safari(apple)
# Different Language incompatibilities	- selenimu search for english text in foreign characters will fail
#
#
import os
import sys
...
def Browser():
  ...
  #
  # chrom-63 - 1024 x 768
  # chrome_options.add_argument("--window-position=150,25")	# no negative numbers
  # chrome_options.add_argument("--window-size=825,675")	# width,height
  #
...
def SavePage( cmdopt, id, url ):
  ...
  for line in data.split( '\n' ):
    ...
    if   line.find( '<title>' ) >= 0 :
      ...
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( '<meta property="og:' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
#   elif line.find( '<meta property="og:title"' ) >= 0 :
#     IgnoreBodyStr ( cmdopt, line )
#     #
#   elif line.find( 'meta property="og:url"' ) >= 0 :
#     IgnoreBodyStr ( cmdopt, line )
#     #
#   elif line.find( 'meta property="og:image"' ) >= 0 :
#     IgnoreBodyStr ( cmdopt, line )
#     #
      ...
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( '<p id="display-date" class="postinginfo reveal">' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )	# BUT, still want following "<time class="
      #
    elif line.find( '<time class="date timeago" ' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
      # <time class="date timeago" datetime="2018-01-15T11:17:41-0800">
      # <p class="postinginfo reveal">posted: <time class="date timeago" datetime="2018-01-15T11:17:41-0800">	# 2018-01-15
      # <p class="postinginfo reveal">updated: <time class="date timeago" datetime="2018-01-29T16:08:24-0800">	# 2018-01-29
      # 
      ind = line.index( "datetime=" )
      #
      p = line.find( "posted:" )
      u = line.find( "updated:" )
      #
	...
	tstr = ""	# ignore <time class= ....
	#
      #
      DateTimeStr = line[ ind+10 : ind+29 ] + " " + tstr	# substr() == extract 2018-mm-dd and hh:mm:ss
      #
      if ( tstr != "" and cmdopt.DEBUG >= 4 ) :
 	prMsg ( "# .. time[%d]=%s..\n", id, DateTimeStr )
	#
      #
      ...
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( 'class="slider-info"' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( 'class="swipe-wrap"' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( 'https://images.craigslist.org' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( 'class="print-qrcode"' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
	...
	#
	# <p class="postinginfo">post id: 6463229684</p>	# 64*.html
	...
	#
	# NOTICE: == this is a duplicate find() of time class=...
	#
	# <p class="postinginfo reveal">posted: <time class="date timeago" datetime="2018-01-17T12:18:22-0800">	 # 2018-01-15
 	# <p class="postinginfo reveal">updated: <time class="date timeago" datetime="2018-01-29T16:08:24-0800"> # 2018-01-29
	#
      IgnoreBodyStr ( cmdopt, line )
    #
    ...
    #
      ...
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( 'class="bestof-link"' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( 'postingID=' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
    elif line.find( 'var imgList = ' ) >= 0 :
      IgnoreBodyStr ( cmdopt, line )
      #
...
def FindDuplicateAds( driver, cmdopt ):
  ...
  for id in range( 0, pcnt):
    ...
    if dup >= 1 :
      fnd += 1
      if cmdopt.DEBUG >= 2 :
	prMsg ( "# .. Duplicate[%d]: %s: %s.. %s..\n", id, DateTime[id], BodyURL[id], TitleStr[id] )
      #
      if flag >= 1 :
	#
  ...
  for id in range( 0, pcnt):
    ...
    if ( hash == "title" ):
 	...
 	prMsg ( "# .. title[%d]=%s.. %s..\n", cnt, Title[id], TitleStr[id] )
      ...
      #
      # Speed things up ... don't calculate body hash - use hash of href instead
      # =============================================
      if ( cmdopt.body == 1 ):
	#
	# ---------------------------------------------
	...
	#
 	# Create unique random bodyHash filler - use href instead
	# ------------------------------------
